#!/usr/bin/env bash
# packs/ironclaw/install.sh — Install IronClaw with native AWS Bedrock support
#
# Usage:
#   ./install.sh [--region us-east-1] [--model us.anthropic.claude-sonnet-4-6-v1]
#
# Assumes:
#   - curl available
#   - IAM role with bedrock:InvokeModel permissions (EC2 instance profile)
#
...
# LLM_BACKEND=bedrock, using the native AWS SDK credential chain.
#
...
Install IronClaw with native AWS Bedrock support.
...
Uses native Bedrock backend (AWS SDK credential chain via EC2 instance profile).

Examples:
  ./install.sh --region us-east-1
  ./install.sh --model us.anthropic.claude-sonnet-4-6-v1
EOF
}
...
# Write .env with native Bedrock + PostgreSQL config — bypasses NEAR AI OAuth entirely
# IronClaw's Bedrock backend expects BEDROCK_MODEL (base model ID without cross-region
# prefix) and BEDROCK_REGION. If the model has a cross-region prefix like "us.", strip
# it and set BEDROCK_CROSS_REGION separately.
BEDROCK_MODEL_ID="${MODEL}"
BEDROCK_CROSS=""
if [[ "${MODEL}" =~ ^(us|eu|apac|global)\. ]]; then
  BEDROCK_CROSS="${BASH_REMATCH[1]}"
  BEDROCK_MODEL_ID="${MODEL#*.}"
fi

cat > "${HOME}/.ironclaw/.env" <<EOF
# IronClaw config — generated by loki-agent pack installer
# LLM: native Bedrock backend (AWS SDK credential chain via EC2 instance profile)
LLM_BACKEND=bedrock
BEDROCK_MODEL=${BEDROCK_MODEL_ID}
BEDROCK_REGION=${REGION}
EOF

# Add cross-region prefix if model ID had one (e.g. us.anthropic.claude-*)
if [[ -n "${BEDROCK_CROSS}" ]]; then
  echo "BEDROCK_CROSS_REGION=${BEDROCK_CROSS}" >> "${HOME}/.ironclaw/.env"
fi
...
log "  LLM_BACKEND=bedrock (native AWS SDK)"
log "  BEDROCK_MODEL=${BEDROCK_MODEL_ID}"
log "  BEDROCK_REGION=${REGION}"
[[ -n "${BEDROCK_CROSS}" ]] && log "  BEDROCK_CROSS_REGION=${BEDROCK_CROSS}"
...
printf "  model: %s via native Bedrock\n" "${MODEL}"
